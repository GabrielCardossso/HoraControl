...
public class TurmaService {
  ...
  public List<Turma> listar() { return repository.findAllByOrderByCodigoAsc(); }
  public List<Turma> listarAtivas() { return repository.findByAtivoTrueOrderByCodigoAsc(); }
  public Turma buscar(Long id) { return repository.findById(id).orElseThrow(() -> new RegraNegocioException("Turma não encontrada.")); }

  @Transactional
  public Turma salvar(Turma turma) {
    if (turma.getCodigo() == null || turma.getCodigo().isBlank()) throw new RegraNegocioException("O código da turma é obrigatório.");
    if (turma.getNome() == null || turma.getNome().isBlank()) throw new RegraNegocioException("O nome da turma é obrigatório.");
    ...
    if (duplicado) throw new RegraNegocioException("Já existe uma turma com este código.");
    ...
    if (turma.getCurso() != null && turma.getCurso().getId() != null)
      turma.setCurso(cursoRepository.findById(turma.getCurso().getId()).orElseThrow(() -> new RegraNegocioException("Curso não encontrado.")));
