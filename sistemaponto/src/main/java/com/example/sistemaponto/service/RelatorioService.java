...
public class RelatorioService {
  ...
  public byte[] gerarExcel(List<RegistroPonto> registros, String titulo) {
    try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      Cell geradoEm = geradoRow.createCell(1); geradoEm.setCellValue(LocalDateTime.now()); geradoEm.setCellStyle(dataHoraExcel);
      ...
      for (int i = 0; i < indicadores.length; i++) {
        ...
        if (i == 2) valor.setCellStyle(decimal);
        if (i == 3) valor.setCellStyle(moeda);
      }
      ...
      for (int i = 0; i < colunas.length; i++) {
        Cell cell = header.createCell(i); cell.setCellValue(colunas[i]); cell.setCellStyle(cabecalho);
      }
      int linha = 1;
      for (RegistroPonto r : registros) {
        ...
        Cell entrada = row.createCell(5); entrada.setCellValue(r.getEntrada()); entrada.setCellStyle(dataHoraExcel);
        Cell saida = row.createCell(6); if (r.getSaida() != null) { saida.setCellValue(r.getSaida()); saida.setCellStyle(dataHoraExcel); }
        int excelRow = row.getRowNum() + 1;
        Cell horas = row.createCell(7); horas.setCellFormula("IF(G" + excelRow + "=\"\",0,ROUND((G" + excelRow + "-F" + excelRow + ")*24,2))"); horas.setCellStyle(decimal);
        Cell valorHora = row.createCell(8); valorHora.setCellValue(r.getTurma() == null ? 0 : r.getTurma().getValorHora().doubleValue()); valorHora.setCellStyle(moeda);
        Cell valor = row.createCell(9); valor.setCellFormula("ROUND(H" + excelRow + "*I" + excelRow + ",2)"); valor.setCellStyle(moeda);
      ...
      dados.setAutoFilter(new org.apache.poi.ss.util.CellRangeAddress(0, Math.max(0, linha - 1), 0, colunas.length - 1));
      int[] larguras = {26, 15, 25, 16, 18, 20, 20, 11, 16, 18, 13, 42};
      for (int i = 0; i < larguras.length; i++) dados.setColumnWidth(i, larguras[i] * 256);
  ...
  public byte[] gerarPdf(List<RegistroPonto> registros, String titulo) {
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      com.lowagie.text.Font titleFont = new com.lowagie.text.Font(base, 18, com.lowagie.text.Font.BOLD, new Color(44, 52, 96));
      ...
      document.add(new Paragraph("Registros: " + registros.size() + "   |   Total de horas: " + totalHoras(registros)
          + "   |   Valor estimado: R$ " + totalValor(registros), new com.lowagie.text.Font(base, 10, com.lowagie.text.Font.NORMAL, Color.DARK_GRAY)));
      ...
      String[] headers = {"Professor", "Turma", "Código", "Entrada", "Saída", "Horas", "Valor/h", "Total", "Observação"};
      for (String h : headers) table.addCell(celula(h, headerFont, new Color(75, 71, 153)));
      for (RegistroPonto r : registros) {
        table.addCell(celula(r.getProfessor().getNome(), body, Color.WHITE));
        table.addCell(celula(r.getTurma() == null ? texto(r.getDescricao()) : r.getTurma().getNome(), body, Color.WHITE));
      ...
      document.add(table);
      document.add(Chunk.NEWLINE);
      document.add(new Paragraph("Valores são estimativas calculadas pelo valor-hora cadastrado na turma. Registros abertos não entram no total financeiro.", body));
      document.close();
      return out.toByteArray();
    } catch (Exception e) {
  ...
  private CellStyle estiloTitulo(Workbook wb) {
    ...
    org.apache.poi.ss.usermodel.Font font = wb.createFont(); font.setBold(true); font.setFontHeightInPoints((short) 18); font.setColor(IndexedColors.DARK_BLUE.getIndex());
  ...
  private CellStyle estiloCabecalho(Workbook wb) {
    ...
    org.apache.poi.ss.usermodel.Font font = wb.createFont(); font.setBold(true); font.setColor(IndexedColors.WHITE.getIndex()); style.setFont(font);
  ...
  private PdfPCell celula(String texto, com.lowagie.text.Font font, Color fundo) {
    ...
    cell.setBackgroundColor(fundo); cell.setPadding(5); cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
    return cell;
  }
}
