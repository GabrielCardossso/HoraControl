...
public class CursoService {
  ...
  public Curso salvar(Curso curso) {
    if (curso.getNome() == null || curso.getNome().isBlank()) throw new RegraNegocioException("O nome do curso é obrigatório.");
    if (curso.getCodigo() == null || curso.getCodigo().isBlank()) throw new RegraNegocioException("O código do curso é obrigatório.");
    boolean duplicado = curso.getId() == null ? repository.existsByCodigoIgnoreCase(curso.getCodigo())
        : repository.existsByCodigoIgnoreCaseAndIdNot(curso.getCodigo(), curso.getId());
    if (duplicado) throw new RegraNegocioException("Já existe um curso com este código.");
